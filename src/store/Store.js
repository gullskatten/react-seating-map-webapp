import {
  UrlUpdateMember,
  UrlFindAllFloors,
  UrlFindAllTeamsByFloorId
} from '../containers/constants/UrlConstants';
...
export default class Store {
  ...
  @action goToSlide(slide) {
    console.log(slide);
  }

  @action fetchAllFloors() {
    return axios
      .get(UrlFindAllFloors)
      .then(response => {
        this.floors = mobxToJS(response.data);
        let isFirst = true;
        this.floors.forEach(floor => {
          axios
            .get(`${UrlFindAllTeamsByFloorId}${floor._id}`)
            .then(resp => {
